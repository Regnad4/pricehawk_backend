...
app.get('/api/products', (req, res) => {
  const products = db.get('products').sortBy(p => -new Date(p.created_at)).value();
  res.json(products);
});
...
app.post('/api/products', async (req, res) => {
  ...
  if (scraped.price) {
    ...
    db.get('price_history').push({ id: histId, product_id: id, price: scraped.price, recorded_at: now }).write();
...
app.post('/api/products/:id/check', async (req, res) => {
  ...
  const now = new Date().toISOString();
  db.get('products').find({ id: req.params.id }).assign({ current_price: result.price, last_checked: now }).write();

  if (result.price) {
    const histId = db.get('_nextHistoryId').value();
    db.get('price_history').push({ id: histId, product_id: req.params.id, price: result.price, recorded_at: now }).write();
    db.set('_nextHistoryId', histId + 1).write();
  }
...
app.get('/api/notifications', (req, res) => {
  const notifications = db.get('notifications').sortBy(n => -new Date(n.created_at)).take(50).value();
