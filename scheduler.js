...
async function checkAllProducts() {
  ...
  for (const product of products) {
    ...
    db.get('price_history').push({ id: histId, product_id: product.id, price: newPrice, recorded_at: now }).write();
    db.set('_nextHistoryId', histId + 1).write();

    if (newPrice <= product.target_price) {
      const message = `Price dropped to $${newPrice.toFixed(2)}! Your target was $${product.target_price.toFixed(2)}.`;
      createNotification(product.id, message, oldPrice, newPrice, globalPushToken);
    } else if (oldPrice && newPrice < oldPrice) {
      const message = `Price dropped from $${oldPrice.toFixed(2)} to $${newPrice.toFixed(2)}.`;
